...
> Дополнительно: если вы используете REST API (скрейпер/сервер) и хотите подключить другой Supabase проект, задайте переменные окружения в корне проекта (файл `.env`) — `SUPABASE_URL` и `SUPABASE_KEY`.
>
> Пример (в корне проекта):
> ```env
> SUPABASE_URL=https://qwrgjwbitlcdapmpmrhv.supabase.co
> SUPABASE_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc...etc
> ```
> После добавления перезапустите `scraper` и `backend`.

...
export const setupObserver = (context, observerCreatedCallback) => {
  ...
  let targetSelector;
  switch (context) {
      case 'tradeOffer': targetSelector = '.card-filter-list__items'; break;
      case 'pack': return;
...
const observeNode = (targetNode, context, targetSelector, observerCreatedCallback) => {
    const observerCallback = debounce(async (mutations) => {
        ...
        for (const mutation of mutations) {
            if (mutation.type === 'childList') {
                ...
                if (['tradeOffer', 'remelt', 'market', 'split', 'deckCreate', 'marketRequestCreate', 'auctions', 'marketCreate'].includes(context)) {
        ...
        if (cardListChanged) {
            ...
            // Счетчики активны (уже проверили в начале), обрабатываем
    ...
    }, 1000); // Увеличенный debounce для предотвращения частых переобработок
